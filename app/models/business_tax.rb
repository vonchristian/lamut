class BusinessTax < ApplicationRecord
  ...
  def self.set_tax(gross_sale)
    ...
    return tax_for_businesses_essentially_consisting_of_sales_of_services_for_a_fee(gross_sale) if gross_sale.business.business_classification_name == "Business hereunder enumerated: Cafes, cafeterias, ice cream and other refreshment parlors, restaurants, soda fountain bars, carinderias or food caterers;\nCommission agents; Lessors, dealers, brokers of real estate; On travel agencies and travel agents;\nBoarding houses, motels, apartments, and condominiums; Subdivision owners/Private Cemeteries and Memorial Parks;\nHospitals, medical/dental/therapeutic clinics, medical/dental laboratories;\nOperators of Cable Network System;\nOperators of computer services establishments;\nGeneral consultancy services;\nAll other similar activities consisting essentially of the sales of services for a fee." && (2_000_000..Float::INFINITY).include?(gross_sale.amount.to_f)
  ...
  def self.tax_wholesalers_distributors_or_dealers_of_any_article_of_commerce_of_whatever_kind_or_nature_in_excess_of_2_000_000(gross_sale)
  ...
  def self.tax_for_retailers(gross_sale)
    if gross_sale.amount <= 4_000_000.00
      tax = gross_sale.amount * 0.02
    else
      tax = gross_sale.amount * 0.01
    end
  end
